...
export async function loader({ request }) {
  ...
  return json({ len: Profile.length });
}
...
export async function action({ request }) {
  ...
  newProfile = await db.models.Profiles.create({
    userId: newUser._id,
    name: "Dima Preda",
    date: new Date(),
    ...
    <div className="seed-page">
      <Form method="POST" className="">
        <h1 className="text-center">
          Are you sure that you want to see your database
        </h1>{" "}
        <br />
        <br />
        <div className="flex justify-evenly content-center">
          <Link
            to="/"
            className=" text-white bg-red-800 rounded py-2 px-4 ml-10"
          >
            No
          </Link>
          <button
            type="submit"
            className="mr-10 text-white bg-green-800 rounded px-4 py-2"
          >
            Seed the database
          </button>
        </div>
